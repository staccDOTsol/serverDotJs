...
var mplTokenVault = require('@metaplex-foundation/mpl-token-vault');
var mplCore = require('@metaplex-foundation/mpl-core');
var BN = require('bn.js');
var mplMetaplex = require('@metaplex-foundation/mpl-metaplex');
var mplTokenMetadata = require('@metaplex-foundation/mpl-token-metadata');
var mplAuction = require('@metaplex-foundation/mpl-auction');
...
var mplTokenVault__namespace = /*#__PURE__*/_interopNamespace(mplTokenVault);
var mplCore__namespace = /*#__PURE__*/_interopNamespace(mplCore);
var BN__default = /*#__PURE__*/_interopDefaultLegacy(BN);
var mplMetaplex__namespace = /*#__PURE__*/_interopNamespace(mplMetaplex);
var mplTokenMetadata__namespace = /*#__PURE__*/_interopNamespace(mplTokenMetadata);
var mplAuction__namespace = /*#__PURE__*/_interopNamespace(mplAuction);
...
class ArweaveStorage {
    ...
    upload(files, mintKey, txid) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const response = yield axios__default["default"].post(this.endpoint, body);
...
const lookup = (url) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { data } = yield axios__default["default"].get(url);
        return data;
    }
    catch (_a) {
...
class PayForFiles extends mplCore.Transaction {
    constructor(options, params) {
        ...
        this.add(web3_js.SystemProgram.transfer({
            fromPubkey: feePayer,
            toPubkey: arweaveWallet !== null && arweaveWallet !== void 0 ? arweaveWallet : new web3_js.PublicKey(mplCore.config.arweaveWallet),
            lamports,
        }));
        fileHashes.forEach((data) => {
            this.add(new web3_js.TransactionInstruction({
                keys: [],
                programId: new web3_js.PublicKey(mplCore.config.programs.memo),
                data,
            }));
        });
    }
...
class CreateMint extends mplCore.Transaction {
    constructor(options, params) {
        const { feePayer } = options;
...
class CreateTokenAccount extends mplCore.Transaction {
    constructor(options, params) {
        const { feePayer } = options;
...
class CreateAssociatedTokenAccount extends mplCore.Transaction {
    constructor(options, params) {
        const { feePayer } = options;
...
class MintTo extends mplCore.Transaction {
    constructor(options, params) {
        const { feePayer } = options;
        const { mint, dest, authority, amount } = params;
        super(options);
        this.add(splToken.Token.createMintToInstruction(splToken.TOKEN_PROGRAM_ID, mint, dest, authority !== null && authority !== void 0 ? authority : feePayer, [], new BN__default["default"](amount).toNumber()));
    }
}
...
function createWrappedAccountTxs(connection, owner, amount = 0) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const closeTokenAccountTx = new mplCore.Transaction().add(splToken.Token.createCloseAccountInstruction(splToken.TOKEN_PROGRAM_ID, account.publicKey, owner, owner, []));
        return { account, createTokenAccountTx, closeTokenAccountTx };
    });
}

function createApproveTxs(args) {
    const { authority = web3_js.Keypair.generate(), account, owner, amount } = args;
    const createApproveTx = new web3_js.Transaction().add(splToken.Token.createApproveInstruction(splToken.TOKEN_PROGRAM_ID, account, authority.publicKey, owner, [], amount));
    const createRevokeTx = new web3_js.Transaction().add(splToken.Token.createRevokeInstruction(splToken.TOKEN_PROGRAM_ID, account, owner, []));
...
const sendTransaction = ({ connection, wallet, txs, signers = [], options, }) => __awaiter(void 0, void 0, void 0, function* () {
    let tx = mplCore.Transaction.fromCombined(txs, { feePayer: wallet.publicKey });
...
const addTokensToVault = ({ connection, wallet, vault, nfts, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const vaultAuthority = yield mplTokenVault.Vault.getPDA(vault);
    ...
    for (const nft of nfts) {
        ...
        const safetyDepositBox = yield mplTokenVault.SafetyDepositBox.getPDA(vault, nft.tokenMint);
        const tokenStoreAccount = web3_js.Keypair.generate();
        ...
        const addTokenTx = new mplTokenVault.AddTokenToInactiveVault(txOptions, {
            vault,
...
const initStore = ({ connection, wallet, isPublic = true, }) => __awaiter(void 0, void 0, void 0, function* () {
    const storeId = yield mplMetaplex.Store.getPDA(wallet.publicKey);
    const tx = new mplMetaplex.SetStore({ feePayer: wallet.publicKey }, {
...
const initStoreV2 = ({ connection, wallet, settingsUri = null, isPublic = true, }) => __awaiter(void 0, void 0, void 0, function* () {
    const storeId = yield mplMetaplex.Store.getPDA(wallet.publicKey);
    const configId = yield mplMetaplex.StoreConfig.getPDA(storeId);
    const tx = new mplMetaplex.SetStoreV2({ feePayer: wallet.publicKey }, {
        admin: new web3_js.PublicKey(wallet.publicKey),
        store: storeId,
        config: configId,
        isPublic,
...
const mintNFT = ({ connection, wallet, uri, maxSupply, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const metadataPDA = yield mplTokenMetadata.Metadata.getPDA(mint.publicKey);
    const editionPDA = yield mplTokenMetadata.MasterEdition.getPDA(mint.publicKey);
    ...
    const metadataData = new mplTokenMetadata.MetadataDataData({
        name,
        symbol,
        uri,
    ...
    const createMetadataTx = new mplTokenMetadata.CreateMetadata({
    ...
    const masterEditionTx = new mplTokenMetadata.CreateMasterEdition({ feePayer: wallet.publicKey }, {
        edition: editionPDA,
        metadata: metadataPDA,
        updateAuthority: wallet.publicKey,
        mint: mint.publicKey,
        mintAuthority: wallet.publicKey,
        maxSupply: maxSupply ? new BN__default["default"](maxSupply) : null,
    });
    const txId = yield sendTransaction({
        connection,
        signers: [mint],
...
const mintEditionFromMaster = ({ connection, wallet, masterEditionMint, updateAuthority } = {}) => __awaiter(void 0, void 0, void 0, function* () {
    const masterPDA = yield mplTokenMetadata.MasterEdition.getPDA(masterEditionMint);
    const masterMetaPDA = yield mplTokenMetadata.Metadata.getPDA(masterEditionMint);
    const masterInfo = yield mplCore.Account.getInfo(connection, masterPDA);
    const masterData = new mplTokenMetadata.MasterEdition(masterPDA, masterInfo).data;
    const editionValue = masterData.supply.add(new BN__default["default"](1));
    const { mint, createMintTx, createAssociatedTokenAccountTx, mintToTx } = yield prepareTokenAccountAndMintTxs(connection, wallet.publicKey);
    ...
    const metadataPDA = yield mplTokenMetadata.Metadata.getPDA(mint.publicKey);
    const editionMarker = yield mplTokenMetadata.EditionMarker.getPDA(masterEditionMint, editionValue);
    const editionPDA = yield mplTokenMetadata.Edition.getPDA(mint.publicKey);
    const newEditionFromMasterTx = new mplTokenMetadata.MintNewEditionFromMasterEditionViaToken({ feePayer: wallet.publicKey }, {
        edition: editionPDA,
        metadata: metadataPDA,
...
const createMetadata = ({ connection, wallet, editionMint, metadataData, updateAuthority } = {}) => __awaiter(void 0, void 0, void 0, function* () {
    const metadata = yield mplTokenMetadata.Metadata.getPDA(editionMint);
    const createMetadataTx = new mplTokenMetadata.CreateMetadata({ feePayer: wallet.publicKey }, {
        metadata,
        metadataData,
        updateAuthority: updateAuthority !== null && updateAuthority !== void 0 ? updateAuthority : wallet.publicKey,
        mint: editionMint,
...
const createMasterEdition = ({ connection, wallet, editionMint, updateAuthority, maxSupply } = {}) => __awaiter(void 0, void 0, void 0, function* () {
    const metadata = yield mplTokenMetadata.Metadata.getPDA(editionMint);
    const edition = yield mplTokenMetadata.MasterEdition.getPDA(editionMint);
    const createMetadataTx = new mplTokenMetadata.CreateMasterEdition({ feePayer: wallet.publicKey }, {
        edition,
        metadata,
        updateAuthority: updateAuthority !== null && updateAuthority !== void 0 ? updateAuthority : wallet.publicKey,
        mint: editionMint,
...
const signMetadata = ({ connection, wallet, editionMint, signer } = {}) => __awaiter(void 0, void 0, void 0, function* () {
    const metadata = yield mplTokenMetadata.Metadata.getPDA(editionMint);
    const signTx = new mplTokenMetadata.SignMetadata({ feePayer: wallet.publicKey }, {
        metadata,
...
const updateMetadata = ({ connection, wallet, editionMint, newMetadataData, newUpdateAuthority, primarySaleHappened, } = {}) => __awaiter(void 0, void 0, void 0, function* () {
    const metadata = yield mplTokenMetadata.Metadata.getPDA(editionMint);
    const updateTx = new mplTokenMetadata.UpdateMetadata({ feePayer: wallet.publicKey }, {
        metadata,
        updateAuthority: wallet.publicKey,
...
const cancelBid = ({ connection, wallet, auction, bidderPotToken, destAccount, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const auctionManager = yield mplMetaplex.AuctionManager.getPDA(auction);
    const manager = yield mplMetaplex.AuctionManager.load(connection, auctionManager);
    ...
    const auctionExtended = yield mplAuction.AuctionExtended.getPDA(vault);
    const bidderPot = yield mplAuction.BidderPot.getPDA(auction, bidder);
    const bidderMeta = yield mplAuction.BidderMetadata.getPDA(auction, bidder);
    const accountRentExempt = yield connection.getMinimumBalanceForRentExemption(splToken.AccountLayout.span);
...
const getCancelBidTransactions = ({ destAccount, bidder, accountRentExempt, bidderPot, bidderPotToken, bidderMeta, auction, auctionExtended, auctionTokenMint, vault, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!destAccount) {
        ...
        const closeTokenAccountInstruction = new mplCore.Transaction().add(splToken.Token.createCloseAccountInstruction(splToken.TOKEN_PROGRAM_ID, account.publicKey, bidder, bidder, []));
    ...
    const cancelBidTransaction = new mplAuction.CancelBid({ feePayer: bidder }, {
        bidder,
...
const placeBid = ({ connection, wallet, amount, auction, bidderPotToken, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const placeBidTransaction = new mplAuction.PlaceBid({ feePayer: bidder }, {
        bidder,
...
const redeemFullRightsTransferBid = ({ connection, wallet, store, auction, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const auctionManager = yield mplMetaplex.AuctionManager.getPDA(auction);
    const manager = yield mplMetaplex.AuctionManager.load(connection, auctionManager);
    const vault = yield mplTokenVault.Vault.load(connection, manager.data.vault);
    const fractionMint = new web3_js.PublicKey(vault.data.fractionMint);
    const auctionExtended = yield mplAuction.AuctionExtended.getPDA(vault.pubkey);
    ...
    const safetyDepositConfig = yield mplMetaplex.SafetyDepositConfig.getPDA(auctionManager, safetyDepositBox.pubkey);
    const transferAuthority = yield mplTokenVault.Vault.getPDA(vault.pubkey);
    const metadata = yield mplTokenMetadata.Metadata.getPDA(tokenMint);
...
const getRedeemFRTBidTransactions = ({ accountRentExempt, bidder, tokenMint, store, vault, auction, auctionManager, auctionExtended, bidRedemption, bidderMeta: bidMetadata, safetyDepositTokenStore, safetyDeposit, fractionMint, safetyDepositConfig, transferAuthority, metadata, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const redeemBidTransaction = new mplMetaplex.RedeemFullRightsTransferBid({ feePayer: bidder }, {
        store,
        vault,
        auction,
        auctionManager,
    ...
    const updatePrimarySaleHappenedViaTokenTransaction = new mplTokenMetadata.UpdatePrimarySaleHappenedViaToken({ feePayer: bidder }, {
        metadata,
        owner: bidder,
...
const getBidRedemptionPDA = (auction, bidderMeta) => __awaiter(void 0, void 0, void 0, function* () {
    return (yield web3_js.PublicKey.findProgramAddress([Buffer.from(mplMetaplex.MetaplexProgram.PREFIX), auction.toBuffer(), bidderMeta.toBuffer()], mplMetaplex.MetaplexProgram.PUBKEY))[0];
});

const redeemPrintingV2Bid = ({ connection, wallet, store, auction, }) => __awaiter(void 0, void 0, void 0, function* () {
    const bidder = wallet.publicKey;
    const { data: { bidState }, } = yield mplAuction.Auction.load(connection, auction);
    const auctionManagerPDA = yield mplMetaplex.AuctionManager.getPDA(auction);
    const manager = yield mplMetaplex.AuctionManager.load(connection, auctionManagerPDA);
    const vault = yield mplTokenVault.Vault.load(connection, manager.data.vault);
    const auctionExtendedPDA = yield mplAuction.AuctionExtended.getPDA(vault.pubkey);
    const [safetyDepositBox] = yield vault.getSafetyDepositBoxes(connection);
    const originalMint = new web3_js.PublicKey(safetyDepositBox.data.tokenMint);
    const safetyDepositTokenStore = new web3_js.PublicKey(safetyDepositBox.data.store);
    const bidderMetaPDA = yield mplAuction.BidderMetadata.getPDA(auction, bidder);
    const bidRedemptionPDA = yield getBidRedemptionPDA(auction, bidderMetaPDA);
    const safetyDepositConfigPDA = yield mplMetaplex.SafetyDepositConfig.getPDA(auctionManagerPDA, safetyDepositBox.pubkey);
    ...
    const newMetadataPDA = yield mplTokenMetadata.Metadata.getPDA(newMint);
    const newEditionPDA = yield mplTokenMetadata.Edition.getPDA(newMint);
    const metadataPDA = yield mplTokenMetadata.Metadata.getPDA(originalMint);
    const masterEditionPDA = yield mplTokenMetadata.MasterEdition.getPDA(originalMint);
    const masterEdition = yield mplTokenMetadata.MasterEdition.load(connection, masterEditionPDA);
    const prizeTrackingTicketPDA = yield mplMetaplex.PrizeTrackingTicket.getPDA(auctionManagerPDA, originalMint);
    let prizeTrackingTicket;
    try {
        prizeTrackingTicket = yield mplMetaplex.PrizeTrackingTicket.load(connection, prizeTrackingTicketPDA);
    }
    catch (e) {
    ...
    const editionMarkerPDA = yield mplTokenMetadata.EditionMarker.getPDA(originalMint, desiredEdition);
    try {
        const editionMarker = yield mplTokenMetadata.EditionMarker.load(connection, editionMarkerPDA);
    ...
    const txBatch = yield getRedeemPrintingV2BidTransactions({
        ...
        editionOffset,
        winIndex: new BN__default["default"](winIndex),
    });
...
const getRedeemPrintingV2BidTransactions = ({ bidder, destination, store, vault, auction, auctionManager, auctionExtended, bidRedemption, bidderMeta: bidMetadata, safetyDepositTokenStore, safetyDeposit, safetyDepositConfig, metadata, newMint, newMetadata, newEdition, masterEdition, editionMarker: editionMark, prizeTrackingTicket, winIndex, editionOffset, }) => __awaiter(void 0, void 0, void 0, function* () {
    const txBatch = new TransactionsBatch({ transactions: [] });
    const redeemPrintingV2BidTx = new mplMetaplex.RedeemPrintingV2Bid({ feePayer: bidder }, {
        store,
        vault,
        auction,
        auctionManager,
    ...
    const updatePrimarySaleHappenedViaTokenTx = new mplTokenMetadata.UpdatePrimarySaleHappenedViaToken({ feePayer: bidder }, {
...
function getEditionOffset(winIndex) {
    const offset = new BN__default["default"](1);
    return offset.add(new BN__default["default"](winIndex));
}
...
const redeemParticipationBidV3 = ({ connection, wallet, store, auction, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const bidder = wallet.publicKey;
    const { data: { bidState, tokenMint: auctionTokenMint }, } = yield mplAuction.Auction.load(connection, auction);
    const auctionManagerPDA = yield mplMetaplex.AuctionManager.getPDA(auction);
    const manager = yield mplMetaplex.AuctionManager.load(connection, auctionManagerPDA);
    const vault = yield mplTokenVault.Vault.load(connection, manager.data.vault);
    const auctionExtendedPDA = yield mplAuction.AuctionExtended.getPDA(vault.pubkey);
    ...
    const bidRedemptionPDA = yield getBidRedemptionPDA(auction, bidderMetaPDA);
    const safetyDepositConfigPDA = yield mplMetaplex.SafetyDepositConfig.getPDA(auctionManagerPDA, safetyDepositBox.pubkey);
    const { data: { participationConfig: { fixedPrice }, }, } = yield mplMetaplex.SafetyDepositConfig.load(connection, safetyDepositConfigPDA);
    const acceptPaymentAccount = new web3_js.PublicKey(manager.data.acceptPayment);
    const { mint, createMintTx, createAssociatedTokenAccountTx, mintToTx, recipient } = yield prepareTokenAccountAndMintTxs(connection, wallet.publicKey);
    ...
    const desiredEdition = masterEdition.data.supply.add(new BN__default["default"](1));
    const editionMarkerPDA = yield mplTokenMetadata.EditionMarker.getPDA(originalMint, desiredEdition);
    let tokenPaymentAccount;
    if (auctionTokenMint === splToken.NATIVE_MINT.toBase58()) {
        ...
        tokenPaymentAccount = yield splToken.Token.getAssociatedTokenAddress(splToken.ASSOCIATED_TOKEN_PROGRAM_ID, splToken.TOKEN_PROGRAM_ID, new web3_js.PublicKey(auctionTokenMint), bidder);
    }
    ...
    const redeemParticipationBidV3Tx = new mplMetaplex.RedeemParticipationBidV3({ feePayer: bidder }, {
        store,
        vault: vault.pubkey,
        auction,
        auctionManager: auctionManagerPDA,
        ...
        prizeTrackingTicket: prizeTrackingTicketPDA,
        winIndex: winIndex !== null ? new BN__default["default"](winIndex) : null,
...
function isEligibleForParticipationPrize(winIndex, { nonWinningConstraint, winnerConstraint } = {}) {
    const noWinnerConstraints = winnerConstraint !== mplMetaplex.WinningConstraint.NoParticipationPrize;
    const noNonWinnerConstraints = nonWinningConstraint !== mplMetaplex.NonWinningConstraint.NoParticipationPrize;
...
const claimBid = ({ connection, wallet, store, auction, bidderPotToken, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const manager = yield mplMetaplex.AuctionManager.load(connection, auctionManager);
    const vault = new web3_js.PublicKey(manager.data.vault);
    const { data: { tokenMint }, } = yield mplAuction.Auction.load(connection, auction);
...
const getClaimBidTransactions = ({ bidder, auctionTokenMint, store, vault, auction, auctionManager, auctionExtended, acceptPayment, bidderPot, bidderPotToken, }) => __awaiter(void 0, void 0, void 0, function* () {
    const txBatch = new TransactionsBatch({ transactions: [] });
    const claimBidTransaction = new mplMetaplex.ClaimBid({ feePayer: bidder }, {
        store,
        vault,
        auction,
        auctionExtended,
...
const instantSale = ({ connection, wallet, store, auction, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const vault = yield mplTokenVault.Vault.load(connection, manager.data.vault);
    const auctionExtendedPDA = yield mplAuction.AuctionExtended.getPDA(vault.pubkey);
    const { data: { instantSalePrice }, } = yield mplAuction.AuctionExtended.load(connection, auctionExtendedPDA);
    const [safetyDepositBox] = yield vault.getSafetyDepositBoxes(connection);
    const safetyDepositConfigPDA = yield mplMetaplex.SafetyDepositConfig.getPDA(auctionManagerPDA, safetyDepositBox.pubkey);
    const { data: { winningConfigType, participationConfig }, } = yield mplMetaplex.SafetyDepositConfig.load(connection, safetyDepositConfigPDA);
    ...
    if (hasWinner) {
        switch (winningConfigType) {
            case mplMetaplex.WinningConfigType.FullRightsTransfer: {
                const { txId } = yield redeemFullRightsTransferBid({ connection, wallet, store, auction });
                txIds.push(txId);
                break;
            }
            case mplMetaplex.WinningConfigType.PrintingV2: {
...
const burnToken = ({ connection, wallet, token, mint, amount, owner, close = true, }) => __awaiter(void 0, void 0, void 0, function* () {
    const tx = new mplCore.Transaction({ feePayer: wallet.publicKey }).add(splToken.Token.createBurnInstruction(splToken.TOKEN_PROGRAM_ID, mint, token, owner !== null && owner !== void 0 ? owner : wallet.publicKey, [], amount));
    if (close) {
...
const sendToken = ({ connection, wallet, source, destination, mint, amount, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        yield mplCore.Account.load(connection, destAta);
    }
    catch (_a) {
    ...
    txs.push(new mplCore.Transaction(transactionCtorFields).add(splToken.Token.createTransferInstruction(splToken.TOKEN_PROGRAM_ID, source, destAta, wallet.publicKey, [], amount)));
...
const closeVault = ({ connection, wallet, vault, priceMint, }) => __awaiter(void 0, void 0, void 0, function* () {
    const accountRent = yield connection.getMinimumBalanceForRentExemption(splToken.AccountLayout.span);
    const fractionMintAuthority = yield mplTokenVault.Vault.getPDA(vault);
    const txBatch = new TransactionsBatch({ transactions: [] });
    const txOptions = { feePayer: wallet.publicKey };
    const { data: { fractionMint, fractionTreasury, redeemTreasury, pricingLookupAddress }, } = yield mplTokenVault.Vault.load(connection, vault);
    ...
    const activateVaultTx = new mplTokenVault.ActivateVault(txOptions, {
        vault,
        numberOfShares: new BN__default["default"](0),
    ...
    const createApproveTx = (account) => new mplCore.Transaction().add(splToken.Token.createApproveInstruction(splToken.TOKEN_PROGRAM_ID, account.publicKey, transferAuthority.publicKey, wallet.publicKey, [], 0));
    txBatch.addTransaction(createApproveTx(payingTokenAccount));
    txBatch.addTransaction(createApproveTx(outstandingShareAccount));
    txBatch.addSigner(transferAuthority);
    const combineVaultTx = new mplTokenVault.CombineVault(txOptions, {
        vault,
...
const createExternalPriceAccount = ({ connection, wallet, }) => __awaiter(void 0, void 0, void 0, function* () {
    const txBatch = new TransactionsBatch({ transactions: [] });
    const txOptions = { feePayer: wallet.publicKey };
    const epaRentExempt = yield connection.getMinimumBalanceForRentExemption(mplTokenVault.Vault.MAX_EXTERNAL_ACCOUNT_SIZE);
    const externalPriceAccount = web3_js.Keypair.generate();
    const externalPriceAccountData = new mplTokenVault.ExternalPriceAccountData({
        pricePerShare: new BN__default["default"](0),
        priceMint: splToken.NATIVE_MINT.toBase58(),
        allowedToCombine: true,
    });
    const uninitializedEPA = new mplCore.Transaction().add(web3_js.SystemProgram.createAccount({
        fromPubkey: wallet.publicKey,
        newAccountPubkey: externalPriceAccount.publicKey,
        lamports: epaRentExempt,
        space: mplTokenVault.Vault.MAX_EXTERNAL_ACCOUNT_SIZE,
        programId: mplTokenVault.VaultProgram.PUBKEY,
    }));
    txBatch.addTransaction(uninitializedEPA);
    txBatch.addSigner(externalPriceAccount);
    const updateEPA = new mplTokenVault.UpdateExternalPriceAccount(txOptions, {
...
const createVault = ({ connection, wallet, priceMint = splToken.NATIVE_MINT, externalPriceAccount, }) => __awaiter(void 0, void 0, void 0, function* () {
    const accountRent = yield connection.getMinimumBalanceForRentExemption(splToken.AccountLayout.span);
    const mintRent = yield connection.getMinimumBalanceForRentExemption(splToken.MintLayout.span);
    const vaultRent = yield connection.getMinimumBalanceForRentExemption(mplTokenVault.Vault.MAX_VAULT_SIZE);
    const vault = web3_js.Keypair.generate();
    const vaultAuthority = yield mplTokenVault.Vault.getPDA(vault.publicKey);
    const txBatch = new TransactionsBatch({ transactions: [] });
    ...
    const uninitializedVaultTx = new mplCore.Transaction().add(web3_js.SystemProgram.createAccount({
        fromPubkey: wallet.publicKey,
        newAccountPubkey: vault.publicKey,
        lamports: vaultRent,
        space: mplTokenVault.Vault.MAX_VAULT_SIZE,
        programId: mplTokenVault.VaultProgram.PUBKEY,
    }));
    txBatch.addTransaction(uninitializedVaultTx);
    txBatch.addSigner(vault);
    const initVaultTx = new mplTokenVault.InitVault({ feePayer: wallet.publicKey }, {
...
const initAuction = ({ connection, wallet, vault, auctionSettings, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const [auctionKey, auctionExtended] = yield Promise.all([
        mplAuction.Auction.getPDA(vault),
        mplAuction.AuctionExtended.getPDA(vault),
    ]);
    const fullSettings = new mplAuction.CreateAuctionArgs(Object.assign(Object.assign({}, auctionSettings), { authority: wallet.publicKey.toBase58(), resource: vault.toBase58() }));
    const auctionTx = new mplAuction.CreateAuction(txOptions, {
        args: fullSettings,
        auction: auctionKey,
...
var index = /*#__PURE__*/Object.freeze({
  ...
  auction: mplAuction__namespace,
  core: mplCore__namespace,
  metaplex: mplMetaplex__namespace,
  metadata: mplTokenMetadata__namespace,
  vault: mplTokenVault__namespace
});
